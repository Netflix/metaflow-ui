...
type TaskLineData = { start: number; end: number; status: 'ok' | 'failed' | 'running' | 'unknown' };
          ...
          {graph.group
            ? steps.map((step) => (
                <MiniTimelineRow
                  ...
                  status={step.isFailed ? 'failed' : 'ok'}
                />
              ))
...
const MiniTimelineLine = styled.div<{ status: 'ok' | 'failed' | 'running' | 'unknown' }>`
  position: relative;
